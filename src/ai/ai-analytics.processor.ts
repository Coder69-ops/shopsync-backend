    ...
    async process(job: Job<any, any, string>): Promise<any> {
        ...
        try {
            ...
            if (conversations.length === 0) {
                this.logger.log(`No conversations found for shop ${shopId} yesterday. Skipping analysis.`);
                return;
            }
            ...
            const logs = conversations.map(conv => ({
                ...
                tag: conv.tags,
                messages: conv.messages.map(msg => `${msg.sender}: ${msg.content}`).join('\n'),
            }));
            ...
            this.logger.error(`Error in AiAnalyticsProcessor for shop ${shopId}: ${error.message}`, error.stack);
            throw error;
        }
    }
}
