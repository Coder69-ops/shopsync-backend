...
export class AuthService {
  ...
  async signUp(
    ...
    if (reqDetails && this.fbCapiQueue) {
      this.fbCapiQueue
        ...
        .catch((err) =>
          console.error('[AUTH] Failed to queue FB CAPI StartTrial:', err),
        );
    }
  ...
  async updateProfile(
    userId: string,
    data: { name?: string; phone?: string; profilePic?: string },
  ) {
    const user = await this.db.user.findUnique({
      where: { id: userId },
      include: { shop: true },
  ...
  async handleFacebookDeletion(signedRequest: string) {
    try {
      ...
      console.log(
        `[AUTH] Processing data deletion for Facebook ID: ${facebookId}`,
      );
