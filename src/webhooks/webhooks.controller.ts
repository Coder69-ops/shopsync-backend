...
export class WebhooksController {
  ...
  constructor(
    ...
    const paddleKey =
      this.configService.get<string>('PADDLE_API_KEY') || 'dummy_key';
    const paddleEnvVar =
      this.configService.get<string>('PADDLE_ENVIRONMENT') ||
      this.configService.get<string>('PADDLE_ENV');
    const paddleEnv =
      paddleEnvVar === 'production'
        ...
        if (shopId) {
          ...
          if (items.length > 0) {
            ...
            const starterMonthly =
              this.configService.get('PADDLE_PRICE_STARTER_MONTHLY') ||
              'pri_01kkct18j6jx04zc1jxvr6g735';
            const starterYearly =
              this.configService.get('PADDLE_PRICE_STARTER_YEARLY') ||
              'pri_01kkct31y08686k3s1n6ah1qyt';
            const proMonthly =
              this.configService.get('PADDLE_PRICE_PRO_MONTHLY') ||
              'pri_01kkct4vxpr0jvc1zmy39w70vg';
            const proYearly =
              this.configService.get('PADDLE_PRICE_PRO_YEARLY') ||
              'pri_01kkct76cja0qf79ffdssh3qdh';
          ...
          const subscriptionId =
            (eventData.data as any).subscriptionId ||
            (eventData.data as any).id;
          const customerId = (eventData.data as any).customerId;

          this.logger.log(
            `Upgrading shop ${shopId} to ${planName} and ACTIVE status.`,
          );
          ...
          const nextBilledAtStr =
            (eventData.data as any).currentBillingPeriod?.endsAt ||
          ...
          await this.prisma.shop.update({
            ...
            data: {
              ...
              plan: planName,
              subscriptionPlan:
                planName === 'BASIC' ? 'Starter' : 'Pro Business',
          ...
          if (eventData.eventType === 'transaction.completed') {
            const amountPaid =
              ((eventData.data as any).details?.totals?.total || 0) / 100;
            const currency =
              (eventData.data as any).currencyCode ||
              (eventData.data as any).currency_code ||
              'USD';
            const paymentTxId =
              eventData.eventId || subscriptionId || Date.now().toString();
            ...
            if (!existingPayment) {
              ...
              this.logger.log(
                `Created payment record for shop ${shopId}, txId: ${paymentTxId} in ${currency}`,
              );
            }
          }
        } else {
          this.logger.warn(
            'Received transaction.completed but no shopId found in customData',
          );
        }
