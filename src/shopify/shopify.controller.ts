...
export class ShopifyController {
  ...
  async connect(
    ...
    if (body.accessToken) {
      ...
      if (!body.accessToken.startsWith('shpat_')) {
        if (body.accessToken.startsWith('shpss_')) {
          throw new BadRequestException(
            'You entered a Shopify API Secret Key (shpss_). Please use the Admin API Access Token (shpat_) instead.',
          );
        }
        throw new BadRequestException(
          'Invalid Shopify Admin API Access Token. It should start with "shpat_".',
        );
      }
      ...
      throw new BadRequestException(
        'Please provide either an Access Token or Client ID/Secret.',
      );
    }
    ...
    try {
      ...
      throw new BadRequestException(
        error.message ||
          'Failed to connect to Shopify. Verify your credentials and URL.',
      );
    }
  }
}
