...
export class ShopifyService {
    ...
    private async getAccessToken(shop: any): Promise<string> {
        ...
        if (shop.shopifyAccessToken && shop.shopifyAccessTokenExpiresAt && new Date(shop.shopifyAccessTokenExpiresAt).getTime() > Date.now() + 300000) {
            return decrypt(shop.shopifyAccessToken);
        }

        // Need to refresh token using Client Credentials
        if (!shop.shopifyClientId || !shop.shopifyClientSecret || !shop.shopifyUrl) {
            throw new BadRequestException('Shopify Partner App credentials (Client ID/Secret) are missing.');
        }

        this.logger.log(`Refreshing Shopify access token for shop: ${shop.id}`);

        try {
            const shopDomain = shop.shopifyUrl.replace(/^https?:\/\//, '').replace(/\/$/, '');
            ...
            this.logger.error(`Failed to refresh Shopify token for shop ${shop.id}: ${error?.response?.data ? JSON.stringify(error.response.data) : error?.message}`);
            throw new InternalServerErrorException('Failed to authenticate with Shopify using Partner credentials.');
        }
    }
    ...
    async testConnection(shopId: string): Promise<boolean> {
        ...
        try {
            ...
            this.logger.error(`Shopify connection test failed for shop ${shopId}: ${error?.message}`);
            throw new InternalServerErrorException('Failed to connect to Shopify. Please check your token and URL.');
        }
    }
            ...
            try {
                ...
                await this.db.$transaction(async (tx) => {
                    for (const p of products) {
                        ...
                        const stock = firstVariant?.inventory_management ? (firstVariant?.inventory_quantity || 0) : 999;
                        ...
                        let baseShopUrl = sUrl.startsWith('http') ? sUrl : `https://${sUrl}`;
                        if (!baseShopUrl.includes('myshopify.com')) baseShopUrl = `https://${baseShopUrl}.myshopify.com`;
                ...
                this.logger.error(`Error syncing Shopify products for shop ${shopId}:`, error?.message);
                throw new InternalServerErrorException(`Product sync failed. Check API limits or connectivity.`);
            }
        }

        this.logger.log(`Completed Shopify product sync for shop ${shopId}. Total synced: ${syncedCount}`);
        return { syncedCount };
    }
     ...
     */
    async pushOrder(shop: any, order: any, orderItems: any[]): Promise<string | null> {
        if (!shop.shopifyUrl) {
        ...
        try {
            ...
            const lineItems = orderItems
                .filter(item => item.product?.platform === 'SHOPIFY' && item.product?.externalId)
                .map(item => ({
            ...
            if (response.data && response.data.order?.id) {
                this.logger.log(`Created Shopify order #${response.data.order.id} for internal order ${order.id}`);
                return String(response.data.order.id);
            }

        } catch (error: any) {
            this.logger.error(`Failed to push order ${order.id} to Shopify: ${JSON.stringify(error?.response?.data) || error?.message}`);
        }
