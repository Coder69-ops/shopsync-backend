...
export class WooCommerceService {
     ...
     */
    private buildClient(wooCommerceUrl: string, wooCommerceKey: string, wooCommerceSecret: string): AxiosInstance {
    ...
    async testConnection(shopId: string): Promise<boolean> {
        ...
        if (!shop || !shop.wooCommerceUrl || !shop.wooCommerceKey || !shop.wooCommerceSecret) {
            throw new BadRequestException('Please configure WooCommerce settings first.');
        }

        const client = this.buildClient(shop.wooCommerceUrl, shop.wooCommerceKey, shop.wooCommerceSecret);
        ...
        try {
            ...
            this.logger.error(`WooCommerce connection test failed for shop ${shopId}: ${error?.message}`);
            ...
            try {
                ...
                this.logger.error(`WooCommerce connection fallback test failed for shop ${shopId}: ${innerError?.message}`);
                throw new InternalServerErrorException('Failed to connect to WooCommerce. Please check your credentials and URL.');
            }
        }
    }
    ...
    async syncProducts(shopId: string): Promise<{ syncedCount: number }> {
        ...
        while (hasMore) {
            try {
                    ...
                    for (const p of products) {
                        ...
                        await tx.product.upsert({
                            ...
                            update: {
                                ...
                                description: p.short_description || p.description?.replace(/(<([^>]+)>)/gi, '') || null,
                ...
                this.logger.error(`Error syncing WooCommerce products (page ${page}) for shop ${shopId}:`, error?.message);
                throw new InternalServerErrorException(`Product sync failed on page ${page}. Check API limits or connectivity.`);
            }
        }

        this.logger.log(`Completed WooCommerce product sync for shop ${shopId}. Total synced: ${syncedCount}`);
        return { syncedCount };
    }
    ...
    async pushOrder(shop: any, order: any, orderItems: any[]): Promise<string | null> {
        if (!shop.wooCommerceUrl || !shop.wooCommerceKey || !shop.wooCommerceSecret) {
        ...
        try {
            ...
            const lineItems = orderItems
                .filter(item => item.product?.platform === 'WOOCOMMERCE' && item.product?.externalId)
                .map(item => ({
            ...
            if (response.data && response.data.id) {
                this.logger.log(`Created WooCommerce order #${response.data.id} for internal order ${order.id}`);
                return String(response.data.id);
            }
        } catch (error: any) {
            this.logger.error(`Failed to push order ${order.id} to WooCommerce: ${error?.response?.data?.message || error?.message}`);
        }
