...
export class WooCommerceController {
    ...
    @Post('connect')
    async connect(@Request() req: any, @Body() body: { url: string; consumerKey: string; consumerSecret: string }) {
        // Save to DB
        ...
        try {
            // Test the connection
            const success = await this.wooCommerceService.testConnection(req.user.shopId);
            ...
            await (this.db.shop as any).update({
                where: { id: req.user.shopId },
                data: { wooCommerceUrl: null, wooCommerceKey: null, wooCommerceSecret: null }
            });
            throw new BadRequestException('Failed to connect to WooCommerce verify your credentials.');
        }
    }
