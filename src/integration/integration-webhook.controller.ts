import { Controller, Post, Body, Headers, HttpCode, Param, Logger, Req, UnauthorizedException, RawBodyRequest } from '@nestjs/common';
...
export class IntegrationWebhookController {
    ...
    async handleWooCommerceWebhook(
        ...
        await this.syncProcessor.processWebhook(shopId, 'WOOCOMMERCE', topic, payload);
        return 'OK';
    }
    ...
    async handleShopifyWebhook(
        ...
        if (calculatedHmac !== hmac) {
            this.logger.warn(`Shopify signature mismatch for shop ${shopId}. Expected ${calculatedHmac}, got ${hmac}`);
