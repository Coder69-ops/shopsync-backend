...
export class SyncProcessor extends WorkerHost {
        ...
        while (hasMore) {
            ...
            for (const p of products) {
                await this.upsertProduct(shopId, 'WOOCOMMERCE', {
                    ...
                    description: p.short_description?.replace(/(<([^>]+)>)/gi, '') || p.description?.replace(/(<([^>]+)>)/gi, ''),
    ...
    private async upsertProduct(shopId: string, platform: 'WOOCOMMERCE' | 'SHOPIFY', data: any) {
        // Generate vector embedding
        const textToEmbed = `Product Name: ${data.name}. Description: ${data.description || ''}`;
        let embeddingValues = await this.embeddingsService.generateEmbedding(textToEmbed);
        ...
        const vectorString = embeddingValues.length > 0 ? `[${embeddingValues.join(',')}]` : null;
    ...
    async processWebhook(shopId: string, platform: 'WOOCOMMERCE' | 'SHOPIFY', topic: string, payload: any) {
        this.logger.log(`Processing real-time sync for ${platform} shop ${shopId}: ${topic}`);

        const action = platform === 'WOOCOMMERCE' ? topic.split('.')[1] : topic.split('/')[1];
        ...
        if (platform === 'WOOCOMMERCE') {
            productData = {
                ...
                description: payload.short_description?.replace(/(<([^>]+)>)/gi, '') || payload.description?.replace(/(<([^>]+)>)/gi, ''),
            ...
            const shopDomain = platformIds['shopify']?.shopDomain || 'store.myshopify.com';
