...
export class FacebookCapiService {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.accessToken = this.configService.get<string>('FACEBOOK_CAPI_TOKEN') || '';
    ...
    async sendStartTrialEvent(
        ...
        if (!this.pixelId || !this.accessToken) {
            this.logger.warn('Facebook CAPI credentials missing in environment variables.');
            return;
        }
            ...
            data: [
                {
                    ...
                    event_source_url: reqParams?.sourceUrl || 'https://আপনার-ওয়েবসাইট.com/register',
                    user_data: {
                    ...
                    custom_data: {
                        currency: 'BDT',
                        value: 0.00, // ট্রায়াল হওয়ায় ভ্যালু 0 হতে পারে
                    },
                },
            ],
        ...
        try {
            ...
            this.logger.log(`[CAPI] StartTrial event sent! FB Trace ID: ${response.data.fbtrace_id}`);
            return response.data;
        } catch (error: any) {
            this.logger.error(
                `[CAPI] Failed to send StartTrial event: ${error.response?.data?.error?.message || error.message
                }`
            );
        }
    }
